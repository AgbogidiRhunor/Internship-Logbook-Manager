...
def student_dashboard(request):
    ...
    recent_entries = DailyLogEntry.objects.filter(student=request.user).order_by('-entry_date')[:5]
    grade = getattr(profile, 'grade_record', None)
    context = {
        'profile': profile, 'today': today,
        'recent_entries': recent_entries, 'grade': grade,
...
def coordinator_dashboard(request):
    ...
    return render(request, 'dashboard/coordinator_dashboard.html', data)
