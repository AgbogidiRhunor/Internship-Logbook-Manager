...
from .models import CustomUser, StudentProfile, LecturerProfile, CoordinatorProfile, UserRole
...
class StudentRegistrationForm(forms.Form):
    ...
    matric_number = forms.CharField(max_length=30, strip=True, validators=[validate_matric_number])
    ...
    internship_duration = forms.ChoiceField(choices=StudentProfile.InternshipDuration.choices)
    ...
    username = forms.CharField(max_length=80, strip=True, validators=[validate_username])
        ...
        if start and end:
            if start >= end:
                ...
                if duration == '3' and not (80 <= days <= 100):
                    self.add_error('internship_end_date', '3-month internship should span approximately 90 days.')
                elif duration == '6' and not (170 <= days <= 190):
                    self.add_error('internship_end_date', '6-month internship should span approximately 180 days.')
    ...
    def save(self):
        ...
        user = CustomUser.objects.create_user(
            ...
            role=UserRole.STUDENT, email=data.get('email', ''),
        )
        StudentProfile.objects.create(
            user=user, surname=data['surname'], other_names=data['other_names'],
            matric_number=data['matric_number'], university=data['university'],
            faculty=data['faculty'], department=data['department'],
            year_of_study=data['year_of_study'], company_name=data['company_name'],
...
class LecturerRegistrationForm(forms.Form):
    ...
    def save(self):
        ...
        user = CustomUser.objects.create_user(
            username=data['username'], password=data['password'],
            role=UserRole.LECTURER, lecturer_approved=False,
            is_active=True, email=data.get('email', ''),
        )
...
class CoordinatorRegistrationForm(forms.Form):
    surname = forms.CharField(max_length=100, strip=True)
    other_names = forms.CharField(max_length=150, strip=True)
    university = forms.ModelChoiceField(queryset=University.objects.filter(is_active=True))
    email = forms.EmailField(max_length=254)
    ...
    def save(self):
        ...
        user = CustomUser.objects.create_user(
            username=data['username'], password=data['password'],
            role=UserRole.COORDINATOR, coordinator_approved=False,
            is_active=True, email=data.get('email', ''),
        )
